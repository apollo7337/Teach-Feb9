...
/**
 * Retry wrapper with exponential backoff for handling rate limits (429 errors)
 */
const withRetry = async <T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      console.warn(
        `Rate limit hit (attempt ${attempt + 1}/${maxRetries + 1}). Retrying in ${delayMs}ms...`,
      );
...
export const transcribeAudio = async (
  ...
  return withRetry(async () => {
    ...
    const response = await ai.models.generateContent({
      model: "gemini-2.5-flash-lite", // Using 2.5 Flash-Lite for better quota availability
      contents: {
        parts: [
          {
            inlineData: {
...
export const generateTeacherScript = async (
  ...
  return withRetry(async () => {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: {
        parts: [{text: systemPrompt.trim()}, {text: prompt.trim()}],
      },
    });
